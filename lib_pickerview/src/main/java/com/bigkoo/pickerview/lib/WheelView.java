...
public class WheelView extends View {
    ...
    // 条目间距倍数
    float lineSpacingMultiplier = 3F;
    boolean isLoop;
    ...
    int measuredHeight1;// WheelView 控件高度
    ...
    private final float DEFAULT_TEXT_TARGET_SKEWX = 1f;
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        textOutSize = getResources().getDimensionPixelSize(R.dimen.pickerview_textoutsize);//默认大小
...
//        halfCircumference = (int) (itemHeight * 3);
        //整个圆的周长除以PI得到直径，这个直径用作控件的总高度
        measuredHeight = (int) ((halfCircumference * 2) / Math.PI);
        measuredHeight1 = (int) (itemHeight * 3);
        //求出半径
        radius = (int) (halfCircumference / Math.PI);
        //控件宽度，这里支持weight
        measuredWidth = MeasureSpec.getSize(widthMeasureSpec);
        //计算横线 和 选中项画笔的基线Y位置
        firstLineY = (measuredHeight1 - itemHeight) / 2.0F;
        secondLineY = (measuredHeight1 + itemHeight) / 2.0F;
        thirdLineY = 0;
        fourLineY = measuredHeight1;
...
//        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (dividerType == DividerType.WRAP) {//横线长度仅包裹内容
            ...
            canvas.drawLine(startX, thirdLineY, endX, thirdLineY, paintIndicator);
            canvas.drawLine(startX, fourLineY, endX, fourLineY, paintIndicator);
        } else {
            canvas.drawLine(0.0F, firstLineY, measuredWidth, firstLineY, paintIndicator);
            canvas.drawLine(0.0F, secondLineY, measuredWidth, secondLineY, paintIndicator);
            canvas.drawLine(0.0F, thirdLineY, measuredWidth, thirdLineY, paintIndicator);
            canvas.drawLine(0.0F, fourLineY, measuredWidth, fourLineY, paintIndicator);
        }

        //只显示选中项Label文字的模式，并且Label文字不为空，则进行绘制
        if (!TextUtils.isEmpty(label) && isCenterLabel) {
            //绘制文字，靠右并留出空隙
            int drawRightContentStart = measuredWidth - getTextWidth(paintCenterText, label)*1;
...
//                canvas.scale(1.0F, (float) Math.sin(radian));
                ...
                if (translateY <= firstLineY && (itemHeight - maxTextHeight+CENTERCONTENTOFFSET)/2 + translateY >= firstLineY) {
                    // 条目经过第一条线
                    canvas.save();
                    canvas.clipRect(0, 0, measuredWidth, firstLineY - translateY);
                 //   canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
                    canvas.drawText(contentText, drawOutContentStart, (itemHeight +CENTERCONTENTOFFSET)/2, paintOuterText);
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int) (itemHeight));
                   // canvas.scale(1.0F, (float) Math.sin(radian) * 1.0F);
                    canvas.drawText(contentText, drawCenterContentStart, (itemHeight +CENTERCONTENTOFFSET)/2, paintCenterText);
                    canvas.restore();
                }
                else if (translateY <= secondLineY && (itemHeight +CENTERCONTENTOFFSET)/2 + translateY >= secondLineY) {
                    // 条目经过第二条线
                    canvas.save();
                ...
                else if (translateY >= firstLineY && (itemHeight +CENTERCONTENTOFFSET)/2 + translateY <= secondLineY) {
                    // 中间条目
                    //canvas.clipRect(0, 0, measuredWidth,   maxTextHeight);
                    //让文字居中
                    float Y = (itemHeight +CENTERCONTENTOFFSET)/2 ;//因为圆弧角换算的向下取值，导致角度稍微有点偏差，加上画笔的基线会偏上，因此需要偏移量修正一下
...
//                    canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT);
                  ...
                  //  paintOuterText.setTextSkewX((textXOffset == 0 ? 0 : (textXOffset > 0 ? 1 : -1)) * (angle > 0 ? -1 : 1) * DEFAULT_TEXT_TARGET_SKEWX * offsetCoefficient);
                    ...
                    if(translateY < firstLineY){
                        canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight- maxTextHeight));
                        canvas.drawText(contentText, drawOutContentStart + textXOffset * offsetCoefficient, (itemHeight + CENTERCONTENTOFFSET- maxTextHeight)/2, paintOuterText);
                    }else if(translateY>secondLineY){
                        canvas.clipRect(0, 0, measuredWidth, (int) (itemHeight));
                        canvas.drawText(contentText, drawOutContentStart + textXOffset * offsetCoefficient, (itemHeight)/2 + CENTERCONTENTOFFSET, paintOuterText);
                    }
                    canvas.restore();
                }
                canvas.restore();
